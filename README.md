 ...
 *Ends the web service session and releases all resources created by the previous `session/start` call.*
 ...
 Note that the `gentab/text` calls do not return JSON, they return a text response in a variety of selected formats such as HTML, XML, CSV, etc, as well as some custom formats. In this case the example code reads the reponse and plain text and does not attempt to interpret it.
